...
int cmp(db a, db b) { return sign(a - b); }  // a<b : -1 ; a==b : 0 ; a>b : 1 ;
int inmid(db a, db b, db c) { return sign(a - c) * sign(b - c) <= 0; }  // c 在 [a, b] 内 
struct P {
    db x, y; P () {} P (db x, db y) : x(x), y(y) {}
    P operator + (const P &b) const { return P(x + b.x, y + b.y); }  // 向量加
    P operator - (const P &b) const { return P(x - b.x, y - b.y); }  // 向量减
    P operator * (db k) const { return P(x * k, y * k); }            // 向量数乘
    P operator / (db k) const { return P(x / k, y / k); }            // 向量数除
    db operator * (P b) { return x * b.x + y * b.y; }                // 点积
    db operator / (P b) { return x * b.y - y * b.x; }                // 叉积
    bool operator == (const P &b) const { return !cmp(x, b.x) && !cmp(y, b.y); }
    bool operator < (const P b) const {
        int a = cmp(x, b.x);
    	return !a ? cmp(y, b.y) == -1 : a == -1 ? : 1 : 0;
    }
    P turn(db k) { return P(x * cos(k) - y * sin(k), x * sin(k) + y * cos(k)); }  // 逆时针旋转
    P turn90() { return P(-y, x); }                                               // 逆时针旋转 90 度
