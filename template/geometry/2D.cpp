...
bool inVal(T a, T x, T b) { return sgn(a - x) * sgn(b - x) <= 0; }  // 数 x 在区间 [a, b] 内（包括边界）
...
struct P {
    ...
    int isUp() const { return sgn(y) == 1 || !sgn(y) && sgn(x) == -1; }                     // 向量是否在 1 2 象限
};
...
bool order(const P &a, const P &b) { return a.isUp() < b.isUp() || a.isUp() == b.isUp() && sgn(a/b) > 0); }  // 将向量按方位角排序
P proj(P p, P a, P b) { return (b-a) * ( (p-a) * (b-a) / (b-a).abs2() ) + a; }   // 点 p 关于直线 ab 的投影点
